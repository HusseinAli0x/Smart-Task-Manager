...
func NewTaskRepository(db *database.DB) interfaces.TaskRepository {
...
func (r *taskRepository) GetByID(ctx context.Context, id uuid.UUID) (*entities.Task, error) {
	...
	// استخدام r.db.QueryRow من الـ Wrapper
...
// fetchTasks uses r.db.Query (Wrapper) instead of the standard library
func (r *taskRepository) fetchTasks(ctx context.Context, query string, args ...any) ([]*entities.Task, error) {
	rows, err := r.db.Query(ctx, query, args...) // تم تحديثها لاستخدام Wrapper
	if err != nil {
		return nil, err
	}
	defer rows.Close()
